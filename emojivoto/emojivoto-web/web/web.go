...
func (app *WebApp) getRegion(w http.ResponseWriter, r *http.Request) {
	// Read environment variable (fallback to "default_value" if not set)
	envValue := os.Getenv("DEPLOYED_REGION")
	if envValue == "" {
		envValue = "No Region"
	}
...
func StartServer(webPort, webpackDevServer, indexBundle string, emojiServiceClient pb.EmojiServiceClient, votingClient pb.VotingServiceClient) {
	...
	handle("/api/region", webApp.getRegion)
